...
function generateMockScript(lead: Lead): string {
  ...
  return `[SLIDE 1 - OPENER]\nHi ${lead.firstName}, thanks for taking a few minutes to watch this. I put this together specifically for ${company} because I know managing a ${industry} operation at your scale comes with unique challenges.\n\n[SLIDE 2 - THE PAIN]\nHere's what we're seeing across companies like ${company}: ${pain} is costing distributors an average of 2-5% of revenue annually. At Nauta, we currently track 92 in-transit shipments in real-time, and our platform flagged 801 delayed shipments this week alone — before they became customer-facing problems.\n\n[SLIDE 3 - HOW NAUTA WORKS]\nNauta sits on top of your existing systems — we don't replace anything, we synchronize everything. Real-time tracking, automated PO management with 250+ active purchase orders, inventory optimization showing $9.1M on-hand with smart safety stock recommendations, and Section 301 tariff exposure monitoring.\n\n[SLIDE 4 - ROI]\nFor a company ${company}'s size, we typically see: 0.25% fill rate improvement, 18% safety stock reduction freeing significant working capital, and a 92% reduction in emergency POs. That translates to millions in recovered revenue and freed cash.\n\n[SLIDE 5 - NEXT STEPS]\nI'd love to show you a live 20-minute demo tailored to ${company}'s specific operations. Would next week work for a quick call?`;
}

function generateMockSlides(lead: Lead): SlideContent[] {
  const company = lead.company;
  return [
    { title: `Built for ${company}`, subtitle: `How Nauta transforms ${lead.companyIntel?.industry || "supply chain"} operations`, bullets: ["One unified control tower for all operations", "Real-time visibility across your entire supply chain", `Purpose-built for ${lead.companyIntel?.industry || "distribution"}`], highlightStat: "3 min read" },
    ...
    { title: "How Nauta Works", subtitle: "One platform. Every system connected.", bullets: ["Sits on top of your existing ERP + WMS", "92 shipments tracked in real-time right now", "Automated PO management (250+ active)", "Section 301 tariff exposure monitoring"], highlightStat: "801 delays caught" },
